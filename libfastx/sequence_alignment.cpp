...
void  SequenceAlignmentResults::print(std::ostream& strm) const
{
	...
	if ( query_start > 0 )
	  strm << query_sequence.substr(0, query_start) ; //MODIF JM-AURY 25-01-2011 strm << query_sequence.substr(0, query_start-1) ;
	//Aligned query part
	...
	//Alignment bars
	if ( delta > 0 )
	  strm << std::string( delta, ' ') ; //MODIF JM-AURY 25-01-2011 strm << std::string( delta-1, ' ') ;
	strm << "(" ;
...
void SequenceAlignment::resize_matrix(size_t width, size_t height)
{
	...
	//Need to always resize the matrix to avoid side effects due to old alignments
	//if ( matrix_width() >= width && matrix_height() >= height ) return ;
